...
export default function Dashboard() {
  ...
  const colorPalette = [
    ...
    { stroke: 'stroke-indigo-500', bg: 'bg-indigo-500' },
      ...
      <main className="flex-grow p-6 md:p-10 space-y-8 overflow-y-auto max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-zinc-400">Overview of your earnings, budget alerts, and category expenditures.</p>
          </div>
          
          <div className="flex items-center space-x-3">
            <span className="text-xs font-semibold px-3 py-1.5 rounded-xl border border-zinc-800 bg-zinc-900/60 text-zinc-300 flex items-center space-x-1.5">
              <Calendar className="w-3.5 h-3.5" />
              <span>Current Month: {mounted ? new Date().toLocaleString('default', { month: 'long' }) : ''}</span>
            </span>
            <Link 
              href="/transactions?add=true" 
              className="h-10 px-4 rounded-xl bg-violet-600 hover:bg-violet-500 text-sm font-semibold flex items-center space-x-2 transition"
            >
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {/* Income */}
              <div className="p-6 rounded-2xl border border-zinc-850 bg-zinc-900/30 backdrop-blur-sm relative overflow-hidden group">
                ...
                <div className="text-3xl font-bold tracking-tight text-white">{currencySymbol}{totalIncome.toFixed(2)}</div>
                <p className="text-xs text-zinc-500 mt-2">All-time earnings tracked</p>
              </div>
              ...
              <div className="p-6 rounded-2xl border border-zinc-850 bg-zinc-900/30 backdrop-blur-sm relative overflow-hidden group">
                ...
                <div className="text-3xl font-bold tracking-tight text-white">{currencySymbol}{totalExpense.toFixed(2)}</div>
                <p className="text-xs text-zinc-500 mt-2">All-time spending tracked</p>
              </div>

              {/* Net Balance */}
              <div className="p-6 rounded-2xl border border-zinc-850 bg-zinc-900/30 backdrop-blur-sm relative overflow-hidden group sm:col-span-2 lg:col-span-1">
                ...
                <div className={`text-3xl font-bold tracking-tight ${netSavings >= 0 ? 'text-emerald-450' : 'text-rose-400'}`}>
                  {currencySymbol}{netSavings.toFixed(2)}
                </div>
                <p className="text-xs text-zinc-500 mt-2">Income minus expenses</p>
              </div>
            </div>
            ...
            <div className="p-6 rounded-2xl border border-zinc-850 bg-zinc-900/10 space-y-6">
              <div>
                ...
                <p className="text-xs text-zinc-450 mt-1">Detailed review of your spending velocity and highest-ticket purchases across timeframes.</p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                {/* 1. Today Card */}
                <div className="p-4 rounded-xl border border-zinc-850 bg-zinc-955/20 backdrop-blur-sm flex flex-col justify-between space-y-3">
                  <div>
                  ...
                  <div className="text-[11px] text-zinc-450 border-t border-zinc-900/60 pt-2">
                    ...
                    {mounted && todayStats.highest ? (
                      <span className="text-zinc-305 font-medium block truncate mt-0.5" title={`${todayStats.highest.category}: ${todayStats.highest.description || '-'}`}>
                      ...
                      <span className="text-zinc-550 italic block mt-0.5">No expense</span>
                    )}
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl border border-zinc-850 bg-zinc-955/20 backdrop-blur-sm flex flex-col justify-between space-y-3">
                  ...
                  <div className="text-[11px] text-zinc-450 border-t border-zinc-900/60 pt-2">
                    ...
                    {mounted && yesterdayStats.highest ? (
                      <span className="text-zinc-305 font-medium block truncate mt-0.5" title={`${yesterdayStats.highest.category}: ${yesterdayStats.highest.description || '-'}`}>
                ...
                <div className="p-4 rounded-xl border border-zinc-850 bg-zinc-955/20 backdrop-blur-sm flex flex-col justify-between space-y-3">
                  ...
                  <div className="text-[11px] text-zinc-450 border-t border-zinc-900/60 pt-2">
                    ...
                    {mounted && thisWeekStats.highest ? (
                      <span className="text-zinc-305 font-medium block truncate mt-0.5" title={`${thisWeekStats.highest.category}: ${thisWeekStats.highest.description || '-'}`}>
                ...
                <div className="p-4 rounded-xl border border-zinc-850 bg-zinc-955/20 backdrop-blur-sm flex flex-col justify-between space-y-3">
                  ...
                  <div className="text-[11px] text-zinc-450 border-t border-zinc-900/60 pt-2">
                    ...
                    {mounted && thisMonthStats.highest ? (
                      <span className="text-zinc-305 font-medium block truncate mt-0.5" title={`${thisMonthStats.highest.category}: ${thisMonthStats.highest.description || '-'}`}>
                ...
                <div className="p-4 rounded-xl border border-zinc-850 bg-zinc-955/20 backdrop-blur-sm flex flex-col justify-between space-y-3">
                  ...
                  <div className="text-[11px] text-zinc-450 border-t border-zinc-900/60 pt-2">
                    ...
                    {mounted && thisYearStats.highest ? (
                      <span className="text-zinc-305 font-medium block truncate mt-0.5" title={`${thisYearStats.highest.category}: ${thisYearStats.highest.description || '-'}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              {totalSpendingVal === 0 ? (
                <div className="h-64 flex flex-col items-center justify-center text-center p-6 border border-dashed border-zinc-800 rounded-xl bg-zinc-900/10">
                  <TrendingDown className="w-10 h-10 text-zinc-600 mb-2" />
                ...
                <div className="flex flex-col space-y-6">
                  ...
                  <div className="flex flex-col items-center justify-center py-2">
                    ...
                    <div className="relative w-44 h-44">
                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                        <circle cx="18" cy="18" r="15.915" fill="none" className="stroke-zinc-850" strokeWidth="2.5" />
                        {(() => {
                          ...
                          return categoryKeys.map((key, i) => {
                            ...
                            return (
                              <circle
                                ...
                                r="15.915"
                                fill="none"
                                className={colorPalette[i % colorPalette.length].stroke}
                                strokeWidth="3"
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-[10px] text-zinc-500 uppercase tracking-wider font-semibold">Spent</span>
                    ...
                    {/* Colored Dots Row directly below chart */}
                    <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 mt-5 text-[11px] text-zinc-400">
                      {categoryKeys.map((key, i) => {
                        const dotColor = colorPalette[i % colorPalette.length].bg;
                        return (
                          <div key={key} className="flex items-center space-x-1.5 bg-zinc-950/20 px-2.5 py-0.5 rounded-full border border-zinc-900/30">
                  ...
                  {/* List Rows */}
                  <div className="w-full divide-y divide-zinc-900/80 border-t border-zinc-900/80 pt-2">
                    {categoryKeys.map((key, i) => {
                      const val = categorySpending[key];
                      ...
                      return (
                        <button
                          ...
                          className="w-full py-4 flex items-center justify-between text-sm group hover:bg-zinc-900/10 px-3 rounded-xl transition cursor-pointer text-left"
                        >
                          <div className="flex items-center space-x-2">
                            <span className="font-semibold text-zinc-300 group-hover:text-white transition-colors">{key}</span>
                            <span className="text-[10px] font-medium text-zinc-500 bg-zinc-900/80 px-1.5 py-0.5 rounded-md border border-zinc-850/50">({pct}%)</span>
                          </div>
                          <div className="flex items-center space-x-3">
                            <span className="font-bold text-white group-hover:text-violet-400 transition-colors">
                              {currencySymbol}{val.toFixed(2)}
                            </span>
                            <ChevronRight className="w-4.5 h-4.5 text-zinc-500 group-hover:text-white transition-colors flex-shrink-0" />
                          </div>
                        </button>
                      );
                    })}
              ...
              <div className="lg:col-span-4 flex flex-col gap-6">
                  ...
                  {budgetAlerts.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {budgetAlerts.map(alert => (
                        <div key={alert.category} className="p-3.5 rounded-xl border border-amber-500/25 bg-amber-500/5 flex items-start space-x-3">
                ...
                <div className="p-6 rounded-2xl border border-violet-600/30 bg-gradient-to-br from-violet-650/10 to-indigo-700/5 relative overflow-hidden flex flex-col justify-between">
                  <div className="absolute top-[-20%] right-[-10%] w-24 h-24 rounded-full bg-violet-600/10 blur-xl pointer-events-none" />
                  ...
                  <Link 
                    href="/chat"
                    className="mt-5 w-full h-10 flex items-center justify-center space-x-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-xs font-semibold transition group"
                  >
            ...
            <div className="p-6 rounded-2xl border border-zinc-850 bg-zinc-900/20 space-y-5">
              <div className="flex items-center justify-between">
                ...
                <Link href="/transactions" className="text-xs font-semibold text-violet-400 hover:underline flex items-center space-x-1">
                  ...
                  <table className="w-full text-left text-sm border-collapse">
                    <thead>
                      <tr className="border-b border-zinc-850 text-zinc-400 text-xs font-semibold uppercase tracking-wider">
                        ...
                        <th className="pb-3.5 font-medium text-right">Amount</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-900/80">
                      {transactions.slice(0, 5).map(tx => (
                        <tr key={tx._id} className="group hover:bg-zinc-900/10">
                          <td className="py-4">
                            <span className="font-semibold text-white">{tx.category}</span>
                          </td>
                          <td className="py-4 text-zinc-400 truncate max-w-[200px]" title={tx.description}>
                            {tx.description || '-'}
                          </td>
                          ...
                          <td className={`py-4 text-right font-semibold ${tx.type === 'income' ? 'text-emerald-450' : 'text-rose-400'}`}>
        ...
        {selectedCategoryForModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-zinc-950/70 backdrop-blur-sm">
            <div className="w-full max-w-2xl border border-zinc-800 bg-zinc-900 rounded-2xl shadow-2xl relative flex flex-col max-h-[85vh]">
              {/* Header */}
              ...
              <div className="p-6 overflow-y-auto space-y-4 flex-grow">
                {(() => {
                  ...
                  return (
                    <div className="border border-zinc-850 rounded-xl overflow-hidden bg-zinc-955/20">
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="border-b border-zinc-850 text-zinc-400 text-xs font-semibold uppercase tracking-wider bg-zinc-950/50">
                        ...
                        <tbody className="divide-y divide-zinc-900/60 text-sm">
                          {filtered.map(tx => (
                            <tr key={tx._id} className="hover:bg-zinc-900/10 transition">
                              ...
                              <td className="p-3.5 pr-4 text-right font-bold text-rose-400">
              ...
              <div className="p-4 border-t border-zinc-850 bg-zinc-950/40 flex justify-end">
                <button
                  onClick={() => setSelectedCategoryForModal(null)}
                  className="px-4.5 h-9 bg-zinc-800 hover:bg-zinc-750 text-xs font-bold text-zinc-200 rounded-xl transition cursor-pointer"
                >
                  Close
                </button>
              </div>
